...
def main(args: UnivaTrainingDenoiseConfig, attn_implementation="sdpa"):
    ...
    if ps3_model is not None:
        ps3_model.to(accelerator.device, dtype=weight_dtype)
        accelerator.print(
    ...
    if ps3_model is not None:
        ps3_model.requires_grad_(False)
    ...
    dataset = dataset_class(
        ...
        siglip_processor=ps3_processor,
    ...
    for epoch in range(first_epoch, args.training_config.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(lvlm_model):
                ...
                if ps3_model is not None and len(siglip_pixel_values) > 0:
                    siglip_pixel_values = siglip_pixel_values.to(
                        device=accelerator.device,
                        dtype=ps3_model.dtype,
                        non_blocking=True,
                    )
                    ...
                    with torch.no_grad():
                        siglip_hidden_states = ps3_model(
                            siglip_pixel_values, num_look_close=1
                        ).last_hidden_state
...
def log_validation(
    ...
    ps3_model: Optional[Callable] = None,
    ps3_processor: Optional[Callable] = None,
    ...
    if image_path:
        ...
        if ps3_processor is not None:
            siglip_pixel_values = image_dict["siglip_pixel_values"].cuda()
            # B is data parallel number, b is image number in a sequence.
            # siglip_pixel_values Bb c h w, flatten in collator
            siglip_hidden_states = ps3_model(siglip_pixel_values, num_look_close=1).last_hidden_state
            # siglip_hidden_states Bb n d
