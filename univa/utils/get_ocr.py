...
def ocr_with_paddle(img):
    if PaddleOCR is None:
        raise ValueError(
            "sudo apt install swig -y && pip install paddleocr==2.7.0.3 paddle-bfloat==0.1.7 paddlepaddle==2.5.2 protobuf==3.20.2"
        )
...
def process_dataframe(df, image_width, image_height):
    ...
    df_filtered["position"] = df_filtered["box"].apply(
        lambda box: calculate_position(box, image_width, image_height)
    )
...
def format_for_text_to_image_condensed(df, image_number):
    ...
    for position, texts in position_to_texts.items():
        ...
        sentences.append(
            f"The texts {text_string} are located at the {position} of the {ordinal} image."
        )
...
def get_ocr_result(img_path: str, img_index: int = 0):
    ...
    formatted_sentence = format_for_text_to_image_condensed(
        df_processed, image_number=img_index
    )
